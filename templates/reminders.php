...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    if ($action === 'add') {
      ...
      // Process links
      $linksJson = null;
      $linkLabels = array_filter(array_map('trim', $_POST['link_labels'] ?? []));
      $linkUrls = array_filter(array_map('trim', $_POST['link_urls'] ?? []));
      if (!empty($linkLabels) && !empty($linkUrls)) {
        $links = [];
        for ($i = 0; $i < min(count($linkLabels), count($linkUrls)); $i++) {
          if ($linkLabels[$i] && $linkUrls[$i]) {
            $links[] = ['label' => $linkLabels[$i], 'url' => $linkUrls[$i]];
          }
        }
        if (!empty($links)) {
      ...
      if ($title === '' || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) {
        ...
        RemindersRepository::create($title, $date, $recurring === 'none' ? 'none' : 'yearly', null, true, $endDate ?: null, $linksJson);
...
<section>
  ...
  <div class="grid-2">
    ...
    <div class="card">
      ...
      <form method="post" style="padding:12px;background:var(--gray-50);border-radius:8px;margin-bottom:16px">
        <input type="hidden" name="_token" value="<?= Csrf::token() ?>" />
        ...
        <details style="margin-top:8px">
          <summary style="cursor:pointer;font-size:0.875rem;color:var(--gray-600);margin-bottom:6px">Enlaces (opcional)</summary>
...
(function(){
  ...
  // Add link row functionality
  window.addLinkRow = function() {
    const container = document.getElementById('links-container');
    const newRow = document.createElement('div');
    newRow.className = 'link-row';
    newRow.style.cssText = 'display:grid;grid-template-columns:1fr 1fr auto;gap:6px;margin-bottom:4px';
    newRow.innerHTML = `
      <input type="text" name="link_labels[]" placeholder="Etiqueta" style="margin:0;font-size:0.875rem;padding:6px 8px" />
      <input type="url" name="link_urls[]" placeholder="https://..." style="margin:0;font-size:0.875rem;padding:6px 8px" />
      <button type="button" onclick="removeLinkRow(this)" class="btn btn-danger btn-sm" style="padding:6px 8px;font-size:0.75rem">×</button>
    `;
