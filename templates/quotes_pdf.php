...
$client = ClientsRepository::find((int)$qt['client_id']);
...
function qmny($n){ return number_format((float)$n, 2, ',', '.'); }
...
function q_pick_contrast(string $hex): string {
    $h = ltrim($hex, '#');
...
$thText = q_pick_contrast((string)$accent);

$root = dirname(__DIR__);
function q_embed_logo(string $logoUrl, string $root): ?string {
...
function q_svg_uri(string $xml): string { return 'data:image/svg+xml;base64,' . base64_encode($xml); }
...
$icoUser = q_svg_uri($xmlUser);
$icoId = q_svg_uri($xmlId);
$icoLocation = q_svg_uri($xmlLocation);
$icoContact = q_svg_uri($xmlContact);

$html = '<!doctype html>
<html lang="es"><head><meta charset="utf-8"><style>
  body{font-family:DejaVu Sans, Arial, sans-serif; font-size:12px; color:#0F172A}
  .header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid ' . qh($primary) . ';padding-bottom:8px;margin-bottom:12px}
  .brand h1{margin:0;font-size:20px;color:' . qh($primary) . '}
  .meta{font-size:12px;text-align:right}
  .grid{display:grid;grid-template-columns:1fr;gap:10px}
  .box{border:1px solid ' . qh($primary) . ';border-radius:8px;padding:8px}
  ...
  thead th{background:' . qh($accent) . '; color:' . qh($thText) . '}
  ...
  .totals .grand{font-size:16px;font-weight:700;background:#F8FAFC;border:1px solid ' . qh($primary) . ';border-radius:8px;padding:12px;text-align:right}
  .right{text-align:right}
  .notes{margin-top:14px;padding:10px;border:1px solid #E2E8F0;border-radius:8px;background:#F8FAFC;font-size:11px;color:#475569}
</style></head><body>';

$logo = $emitter['logo_url'] ?? '';
$logoSrc = q_embed_logo((string)$logo, $root);

$html .= '<div class="header">'
  . '<div class="brand"><h1>Presupuesto ' . qh($number) . '</h1></div>'
  . '<div class="meta">'
    . ($logoSrc ? ('<div><img src=\'' . qh($logoSrc) . '\' style=\'max-height:60px\' /></div>') : '')
    . '<div><strong>Fecha:</strong> ' . qh($issue) . '</div>'
    . ($validUntil ? '<div><strong>Válido hasta:</strong> ' . qh($validUntil) . '</div>' : '')
  . '</div>'
  . '</div>';
...
$html .= '<strong>Emisor</strong>'
    . '<div class="row"><img class="icon" src="' . qh($icoUser) . '" alt="" /><span>' . qh($emitterName) . '</span></div>'
    . ($emitterNif ? '<div class="row"><img class="icon" src="' . qh($icoId) . '" alt="" /><span>NIF: ' . qh($emitterNif) . '</span></div>' : '')
    . ($emitterAddress !== '' ? '<div class="row"><img class="icon" src="' . qh($icoLocation) . '" alt="" /><span>' . qh($emitterAddress) . '</span></div>' : '')
    . (($emitterEmail || $emitterPhone) ? '<div class="row"><img class="icon" src="' . qh($icoContact) . '" alt="" /><span>' . qh($emitterEmail) . ($emitterPhone ? ' ' . qh($emitterPhone) : '') . '</span></div>' : '')
    . '</div>';

$html .= '<div class="box">'
    . '<strong>Cliente</strong>'
    . '<div class="row"><img class="icon" src="' . qh($icoUser) . '" alt="" /><span>' . qh($clientName) . '</span></div>'
    . ($clientNif !== '' ? '<div class="row"><img class="icon" src="' . qh($icoId) . '" alt="" /><span>NIF: ' . qh($clientNif) . '</span></div>' : '')
    . ($clientAddress !== '' ? '<div class="row"><img class="icon" src="' . qh($icoLocation) . '" alt="" /><span>' . qh($clientAddress) . '</span></div>' : '')
    . (($clientEmail || $clientPhone) ? '<div class="row"><img class="icon" src="' . qh($icoContact) . '" alt="" /><span>' . qh($clientEmail) . ($clientPhone ? ' ' . qh($clientPhone) : '') . '</span></div>' : '')
    . '</div>';
$html .= '</div>';
...
foreach ($items as $it) {
    ...
    $lineTotal = $lineBase + ($lineBase * ($vatRate/100.0)) - ($lineBase * ($irpfRate/100.0));
    $html .= '<tr>'
        . '<td>' . qh($it['description']) . '</td>'
        . '<td>' . qh($qty) . '</td>'
        . '<td>' . qmny($price) . ' €</td>'
        . '<td>' . qh(number_format($vatRate, 2)) . '</td>'
        . '<td>' . qh(number_format($irpfRate, 2)) . '</td>'
        . '<td class="right">' . qmny($lineTotal) . ' €</td>'
        . '</tr>';
}
$html .= '</tbody></table>';
...
$html .= '<div class="totals">
  <div></div>
  <div class="card">
    <div class="right"><strong>Base:</strong> ' . qmny($totals['base']) . ' €</div>
    <div class="right"><strong>IVA:</strong> ' . qmny($totals['iva']) . ' €</div>
    <div class="right after-irpf"><strong>IRPF:</strong> ' . qmny($totals['irpf']) . ' €</div>
    <div class="grand">Total presupuesto: ' . qmny($totals['total']) . ' €</div>
  </div>
</div>';

if (trim($notes) !== '') {
    $html .= '<div class="notes"><strong>Notas:</strong><br />' . nl2br(qh($notes)) . '</div>';
}
...
$filename = 'Presupuesto-' . ($qt['quote_number'] ?: 'borrador-' . $id) . '.pdf';
